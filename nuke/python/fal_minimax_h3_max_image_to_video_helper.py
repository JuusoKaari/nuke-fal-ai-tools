# Purpose:
# - Python 3 helper for Nuke (Python 2.7) to run fal.ai MiniMax H3 Max *image-to-video*.
# - Uploads local start (and optional end) images, calls `minimax/h3-max/image-to-video`, downloads the mp4.
#
# Usage (example):
#   py -3 fal_minimax_h3_max_image_to_video_helper.py --image "C:/start.png" --prompt "..." --out "C:/temp/out.mp4" --verbose
#
# Requirements:
...
_ENDPOINT_ID = "minimax/h3-max/image-to-video"
_DURATION_CHOICES = list(range(5, 16))
_RESOLUTION_CHOICES = ["480P", "768P"]
_PROMPT_EXPANSION_MODE_CHOICES = ["disabled", "balanced", "quality"]


def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description="Run MiniMax H3 Max image-to-video via fal.ai and download the resulting mp4."
    )
    ...
    parser.add_argument(
        ...
        help="Native generation resolution. Default: 768P.",
    )
    parser.add_argument("--seed", type=int, default=None, help="Optional RNG seed.")
    parser.add_argument(
        "--prompt-expansion-mode",
        default="balanced",
        choices=_PROMPT_EXPANSION_MODE_CHOICES,
        help="Prompt rewrite effort: disabled, balanced (~1s), or quality (up to ~30s). Default: balanced.",
    )
    parser.add_argument(
    ...
    user_agent = "nuke-fal-minimax-h3-max-i2v-helper"
    ...
    try:
        ...
        print(
            "ERROR: MiniMax H3 Max image-to-video request failed.\n%s"
