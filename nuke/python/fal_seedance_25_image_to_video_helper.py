# Purpose:
# - Python 3 helper for Nuke (Python 2.7) to run fal.ai ByteDance Seedance 2.5 *image-to-video*.
# - Uploads local start (and optional end) images, calls `bytedance/seedance-2.5/image-to-video`, downloads the mp4.
#
# Usage (example):
#   py -3 fal_seedance_25_image_to_video_helper.py --image "C:/start.png" --prompt "..." --out "C:/temp/out.mp4" --verbose
#
# Requirements:
...
_ENDPOINT_ID = "bytedance/seedance-2.5/image-to-video"
_DURATION_CHOICES = ["auto"] + [str(i) for i in range(4, 31)]


def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description="Run Seedance 2.5 image-to-video via fal.ai and download the resulting mp4."
    )
    ...
    parser.add_argument(
        ...
        help='Duration: "auto" or 4-30 seconds (string values as required by the API). Default: auto.',
    )
    parser.add_argument(
        "--resolution",
        default="720p",
    ...
    user_agent = "nuke-fal-seedance-25-i2v-helper"
    ...
    try:
        ...
        print(
            "ERROR: Seedance 2.5 image-to-video request failed.\n%s"
