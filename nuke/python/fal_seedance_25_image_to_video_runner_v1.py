# Purpose:
# - Runner script for the Nuke Group node `Seedance_25_Image_To_Video_v1` (executes inside Nuke / Python 2.7).
...
# - Calls the external Python 3 helper `fal_seedance_25_image_to_video_helper.py` via subprocess, then creates
...
def main():
    ...
    out_path = os.path.join(out_dir, "seedance_25_i2v_%s.mp4" % ts)
    ...
    returncode, _stdout_lines = runner_util.run_group_helper(
        nuke, g, extra_args, 'Seedance 2.5 Image to Video'
    )

    display_path = video_out.spawn_video_output_read(
        nuke, g, out_path, "Seedance 2.5 image-to-video", "%s_%s" % (g.name(), ts)
    )

    if _nuke_runner_launcher.should_show_success_popup(g):
        nuke.message("Seedance 2.5 image-to-video output created:\n%s" % display_path)
