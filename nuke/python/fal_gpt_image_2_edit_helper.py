# Purpose:
# - Python 3 helper for Nuke (Python 2.7) to run fal.ai OpenAI GPT Image 2 *image editing*.
# - Uploads local reference image(s) to fal storage, calls `openai/gpt-image-2/edit`, downloads result image(s)
#   into a specified output directory, and prints progress/logs to stdout (so Nuke's Script Editor shows it).
#
# Usage (example):
#   py -3 fal_gpt_image_2_edit_helper.py --image "C:/in.png" --prompt "Add sunset lighting" --out-dir "C:/temp/run" --verbose
#
# Requirements:
...
_ENDPOINT_ID = "openai/gpt-image-2/edit"
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description="Run OpenAI GPT Image 2 edit via fal.ai and download results."
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Local reference image path (repeatable, at least one required).",
    )
    parser.add_argument("--prompt", required=True, help="Text prompt describing the desired edit.")
    parser.add_argument("--mask", default="", help="Optional local mask image path (uploaded as mask_url).")
    ...
    if not image_paths:
        print("ERROR: at least one --image is required for GPT Image 2 edit.", file=sys.stderr)
        return 2
    ...
    user_agent = "nuke-fal-gpt-image-2-edit-helper"
    ...
    if result is None:
        print(
            "ERROR: GPT Image 2 edit request failed.\n%s"
