# Purpose:
# - Python 3 helper for Nuke (Python 2.7) to run fal.ai ByteDance Seedance 2.0 *image-to-video*.
# - Uploads local start (and optional end) images, calls `bytedance/seedance-2.0/image-to-video`, downloads the mp4.
#
# Usage (example):
#   py -3 fal_seedance_2_image_to_video_helper.py --image "C:/start.png" --prompt "..." --out "C:/temp/out.mp4" --verbose
#
# Requirements:
...
_ENDPOINT_ID = "bytedance/seedance-2.0/image-to-video"
_DURATION_CHOICES = ["auto"] + [str(i) for i in range(4, 16)]


def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description="Run Seedance 2.0 image-to-video via fal.ai and download the resulting mp4."
    )
    ...
    user_agent = "nuke-fal-seedance-2-i2v-helper"
    ...
    if result is None:
        print(
            "ERROR: Seedance 2.0 image-to-video request failed.\n%s"
