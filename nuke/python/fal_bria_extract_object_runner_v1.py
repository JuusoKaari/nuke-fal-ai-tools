# Purpose:
# - Runner script for the Nuke Group node `Bria_Extract_Object_v1` (executes inside Nuke / Python 2.7).
...
# - Calls `fal_bria_extract_object_helper.py` (Python 3), then wires the RGBA cutout into the baked
...
def _find_output_path(out_dir):
    candidates = [
        os.path.join(out_dir, "extracted.png"),
        os.path.join(out_dir, "extracted.webp"),
        os.path.join(out_dir, "extracted.jpg"),
        os.path.join(out_dir, "extracted.jpeg"),
    ]
...
def main():
    ...
    autocrop = bool(g.knob("autocrop").value())
    remove_background = bool(g.knob("remove_background").value())
    output_mask = bool(g.knob("output_mask").value())

    temp_dir, out_dir, ts = prerender.make_run_dirs(
        nuke_module=nuke,
        prefix="bria_extract_object",
        group_node=g,
    )
    ...
    if autocrop:
        ...
        extra_args += ["--no-autocrop"]
    if remove_background:
        extra_args += ["--remove-background"]
    else:
        extra_args += ["--no-remove-background"]
    if output_mask:
        extra_args += ["--output-mask"]

    returncode, _stdout_lines = runner_util.run_group_helper(
        nuke, g, extra_args, "Bria Extract Object"
    )

    out_path = _find_output_path(out_dir)
    if not out_path:
    ...
    if spawn_reads:
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                r.knob("label").setValue("Bria Extract Object\n%s" % out_path_nk)
            except Exception:
                pass
            r.setXpos(fx)
            r.setYpos(fy)
    ...
    if _nuke_runner_launcher.should_show_success_popup(g):
        nuke.message("Bria Extract Object output created:\n%s" % out_path_nk)
