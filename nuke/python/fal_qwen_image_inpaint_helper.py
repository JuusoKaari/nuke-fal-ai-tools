# Purpose:
# - Python 3 helper for Nuke (Python 2.7) to run fal.ai Qwen Image Edit *inpainting* on a still image + mask.
# - Uploads local image and mask to fal storage, calls `fal-ai/qwen-image-edit/inpaint`, downloads result image(s).
#
# Usage (example):
#   py -3 fal_qwen_image_inpaint_helper.py --image "C:/in.png" --mask "C:/mask.png" --prompt "..." --out-dir "C:/temp/run" --verbose
#
# Requirements:
...
_ENDPOINT_ID = "fal-ai/qwen-image-edit/inpaint"
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description="Run Qwen Image Edit inpaint on a still image + mask via fal.ai and download results."
    )
    ...
    parser.add_argument("--mask", required=True, help="Path to local inpaint mask image (white = edit region).")
    parser.add_argument("--prompt", required=True, help="Text prompt for the inpainted region.")
    ...
    parser.add_argument(
        "--image-size",
        default="",
        help="Optional output size (fal image_size). Leave empty for model default.",
    )
    parser.add_argument(
        "--num-inference-steps",
        type=int,
        default=30,
        help="Denoising steps (2..50). Default: 30.",
    )
    parser.add_argument(
        "--guidance-scale",
        type=float,
        default=4.0,
        help="CFG scale (0..20). Default: 4.",
    )
    parser.add_argument(
        "--strength",
        type=float,
        default=0.93,
        help="Inpaint noise strength (0.01..1). Default: 0.93.",
    )
    parser.add_argument(
        "--acceleration",
        default="regular",
        choices=["none", "regular", "high"],
        help="Speed/quality tradeoff. Default: regular.",
    )
    parser.add_argument(
    ...
    if steps < 2 or steps > 50:
        print("ERROR: --num-inference-steps must be in range 2..50", file=sys.stderr)
        return 2

    guidance = float(args.guidance_scale)
    if guidance < 0.0 or guidance > 20.0:
        print("ERROR: --guidance-scale must be in range 0..20", file=sys.stderr)
        return 2

    strength = float(args.strength)
    if strength < 0.01 or strength > 1.0:
        print("ERROR: --strength must be in range 0.01..1", file=sys.stderr)
        return 2
    ...
    for idx, item in enumerate(images, start=1):
        ...
        if not url:
            continue
        out_name = "image_%03d.%s" % (idx, "jpg" if output_format == "jpeg" else output_format)
