# Purpose:
# - Runner script for the Nuke Group node `MiniMax_H3_Image_To_Video_v1` (executes inside Nuke / Python 2.7).
...
# - Calls the external Python 3 helper `fal_minimax_h3_image_to_video_helper.py` via subprocess, then creates
...
def main():
    ...
    resolution = (g.knob("resolution").value() or "2K").strip()
    ...
    out_path = os.path.join(out_dir, "minimax_h3_i2v_%s.mp4" % ts)
    ...
    returncode, _stdout_lines = runner_util.run_group_helper(
        nuke, g, extra_args, 'MiniMax H3 Image to Video'
    )

    display_path = video_out.spawn_video_output_read(
        nuke, g, out_path, "MiniMax H3 image-to-video", "%s_%s" % (g.name(), ts)
    )

    if _nuke_runner_launcher.should_show_success_popup(g):
        nuke.message("MiniMax H3 image-to-video output created:\n%s" % display_path)
