...
Current coverage (do not duplicate): Nano Banana 2 generate+edit, GPT Image 2 edit, Qwen Max edit / inpaint / layered, Finegrain eraser, BiRefNet still+video, Depth Anything v2, Hunyuan World, Seedance 2.0 I2V, LTX 2.3 + LTX 2.5 Pro I2V, Pika 2.2 Pikaframes, Kling O3 V2V edit, Veo 3.1 extend, ByteDance video upscale, DreamActor v2, Hunyuan 3D v3.1 Pro.
...
3. **Clone** the files in the clone map. Copy helper argparse + `fal_common.subscribe_with_retry`, runner prerender/spawn, and `.nk` knob/input layout. Rename group node, prefixes, output filenames, and knob names to match this tool.
4. **Add the trio**
...
|----------|--------|-------------------|--------------------|-------|
| Seedance 2.5 Image to Video | `seedance` (video) | `fal_seedance_2_image_to_video_{helper.py,runner_v1.py}` + `fal_seedance_2_image_to_video_v1.nk` | Input 0 start still, optional input 1 end still, prompt, duration/resolution/audio knobs, spawn Read for mp4 | Closest upgrade. Extend duration choices to 4-30s (or whatever schema allows). Same Nuke shape. Creates the Seedance submenu. |
| Seedance 2.0 Reference to Video | `seedance` (video) | Helper/knobs/output from Seedance 2.0 I2V. Multi-image inputs from `fal_pika_v22_pikaframes_*`. Video inputs from `fal_bytedance_video_upscale_*` or `fal_dreamactor_v2_*` | Repeated still inputs + optional video (and audio if schema has it), one mp4 out | Schema first: up to 9 images, 3 videos, 3 audio. Do not invent 9+3+3 pipes if the API uses arrays. Named inputs like GPT Image 2 Edit (`fal_gpt_image_2_edit_*`) if labels matter more than order. |
| FLUX 3 First/Last Frame to Video | `flux` (video) | Seedance 2.0 I2V trio | Two stills in, video out | Optional draft endpoint as a knob (`.../first-last-frame-to-video/draft`), not a second node. Keep first/last as its own node (do not merge with single-image FLUX 3 I2V). Flat until keyframes ships. |
| FLUX 3 Keyframes to Video | `flux` (video) | `fal_pika_v22_pikaframes_{helper.py,runner_v1.py}` + `fal_pika_v22_pikaframes_v1.nk` | Contiguous keyframe inputs 0-N, stop at first gap, one mp4 out | Confirm max keyframe count from schema. Optional draft knob. Creates the FLUX 3 submenu. Pika can stay until this ships. |
| MiniMax H3 Image to Video | `minimax` (video) | Seedance 2.0 I2V trio (or LTX 2.5 Pro: `fal_ltx_25_image_to_video_pro_*`) | Start still, optional end still, video out | Resolution defaults toward 2K if schema allows. Same start/end layout as Seedance. Stays flat until a second MiniMax node. |
...
| Veo 3.1 First-Last | `veo` (video) | Seedance 2.0 I2V for start/end stills. `fal_veo3_1_extend_video_*` for Veo helper/output constraints | Two stills in, Veo mp4 out | Creates the Veo submenu next to extend. Copy Veo-specific output limits from the extend helper, not from Seedance. |
| Topaz video interpolate | `utility` (video) | `fal_bytedance_video_upscale_{helper.py,runner_v1.py}` + `fal_bytedance_video_upscale_v1.nk` | Video in, video out, fps/model enums | Slow-mo / 24->60. Video Utility, not a Topaz family. Complements ByteDance upscale, do not merge them. |
...
- [ ] **Seedance 2.5 Image to Video** -- `bytedance/seedance-2.5/image-to-video` -- family `seedance` / video -- upgrade of Seedance 2.0 I2V; 4-30s; optional end frame
- [ ] **Seedance 2.0 Reference to Video** -- `bytedance/seedance-2.0/reference-to-video` -- family `seedance` / video -- up to 9 images, 3 videos, 3 audio; character/set lock
- [ ] **FLUX 3 First/Last Frame to Video** -- `blackforestlabs/flux-3/first-last-frame-to-video` -- family `flux` / video -- start+end stills; optional draft endpoint as a knob
- [ ] **FLUX 3 Keyframes to Video** -- `blackforestlabs/flux-3/keyframes-to-video` -- family `flux` / video -- multi-keyframe; modern Pikaframes
